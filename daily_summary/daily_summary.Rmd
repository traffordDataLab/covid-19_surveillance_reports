...
library(tidyverse) ; library(sf) ; library(lubridate) ; library(zoo) ; library(reactable) ; library(classInt) ; library(leaflet) ; library(htmltools) ; library(htmlwidgets)

# UPDATE POSITIVITY RATE!! #
# Source: PHE Daily COVID-19 Surveillance Reports
fortnight_ending <- as.Date("2020-09-16")
positivity <- tribble(
  ~pillar, ~cases, ~tests,
  "Pillar 1", NA, NA,
  "Pillar 2", NA, NA,
  "Total", NA, NA) %>% 
  mutate(case_rate = round(cases/237354*100000,1),
         test_rate = round(tests/237354*100000,0),
         positivity_rate = round(cases/tests*100,1)) %>% 
  select(pillar, tests, test_rate, positivity_rate)
...
*This summary draws on a number of PHE data sources which have different publication times so the most recent reporting date will vary slightly.*
...
historic_cases <- read_csv("../data/data.csv") %>% 
  ...
  ungroup() %>% 
  fill(date) %>% 
  mutate(ma_cases = rollmean(n, 7, align = "left", fill = NA))
...
weekly_cases <- phe_dashboard %>% 
  ...
  ungroup() %>%
  select(area_name, previous_week_rate, current_week_rate, change_rate) %>% filter(area_name != c("Greater Manchester", "England"))
...
div(class = "subtitle",
    div(class = "title", "Weekly incidence per 100,000 population by local authority"),
    paste("Greater Manchester (England = grey line),", format(max(phe_dashboard$date-weeks(10)), "%d %B"), "to", format(max(phe_dashboard$date-days(2)), "%d %B %Y")))
...
ward_cases <- cases %>% 
  ...
  mutate(previous_week_rate = round(previous_week/population*100000,1),
         ...
         change_rate = round(change/population*100000,1)) %>% 
  ungroup() %>% 
  select(wd19nm, previous_week_rate, current_week_rate, change_rate)
...
breaks <- classIntervals(msoa[msoa$rate > 0, ]$rate, n = 5, style = "jenks")$brks
...
spacing = 17
breaks = seq(0, max(pyramid$rate), by = 20)
...
# Ethnicity
div(class = "subtitle",
    div(class = "title", "Incidence per 100,000 population by ethnicity"),
    paste("Trafford,", format(max(cases$date)-days(6), "%d %B"), "to", format(max(cases$date), "%d %B %Y")))
...
left_join(ethnicity, ethnic_group, by = "ethnic_group") %>% 
  mutate(rate = n/population*100000,1) %>% 
  ggplot() +
  geom_col(aes(x = rate, y = fct_reorder(ethnic_group, rate)), fill = "#CB6598", width = 0.8) +
  geom_vline(xintercept = 0, size = 1, colour = "#333333") +
  geom_label(aes(x = rate, y = ethnic_group, label = round(rate, 0)),
             hjust = 1,  vjust = 0.5, 
             colour = "#FFFFFF", fill = NA, label.size = NA, 
             size = 6) +
...
div(class = "caption", p("Source: PHE Covid-19 Situational Awareness Explorer | 2011 Census"))
```
</div>
</div>
</div>
